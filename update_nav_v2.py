...
nav_template = '''    <nav id="main-nav" class="fixed top-0 w-full bg-gradient-to-b from-black/60 to-black/0 z-50 transition-all duration-300 pointer-events-none">
        ...
        <div id="mobile-menu" class="fixed inset-0 bg-black/95 backdrop-blur-lg z-40 hidden flex-col items-center justify-center pt-20 pb-8 px-6 overflow-y-auto h-screen pointer-events-auto">
            ...
            <div class="flex flex-col items-center gap-8 text-xl w-full max-w-sm mt-8">
...
for f in files:
    ...
    kwargs = {
        ...
        'm_class_home': 'text-brand-primary' if f == 'index.html' else 'text-white/80',
        'm_class_about': 'text-brand-primary' if f == 'about.html' else 'text-white/80',
        'm_class_services': 'text-brand-primary' if f == 'services.html' else 'text-white/80',
        'm_class_portfolio': 'text-brand-primary' if f == 'portfolio.html' else 'text-white/80',
        'm_class_contact': 'text-brand-primary' if f == 'contact.html' else 'text-white/80',
    }
